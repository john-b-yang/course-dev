...
// MARK: CSRF Fix Code Begins Here
...
app.get('/', requiredLogin, csrfProtection, function(req, res, next) {
  let user = req.session.user.name;
  res.render('home', {
    username: user,
    balance: balances[user] || 0,
    balances: balances,
...
app.get('/transfer/', requiredLogin, csrfProtection, function(req, res, next) {
